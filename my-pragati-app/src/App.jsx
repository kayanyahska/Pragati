...
// --- HELPER FUNCTIONS ---
// ✨ THE FINAL FIX: This function now has the correct, full path for the "Individual" view.
const getTaskCollectionPath = (userId, viewMode, groupId) => {
  if (!userId) return null;

  if (viewMode === 'private') {
      // This was the incorrect line. It has been restored to the full path.
    ...
    return (
        <div className="mt-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-2 mb-4">
                {comments.length === 0 ? <p className="text-sm text-gray-500 text-center py-4">Be the first to leave a comment.</p> : comments.map(c => (
                    <div key={c.id} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                        <div className="flex justify-between items-center text-xs mb-1">
                            <span className="font-semibold text-blue-700">{c.createdBy}</span>
            ...
            <form onSubmit={handleAddComment} className="flex space-x-2">
                <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} rows="2" className="flex-grow p-2 border bg-white border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none text-sm text-gray-800 placeholder-gray-400" placeholder="Write a comment..." />
                <button type="submit" disabled={!newComment.trim()} className="self-end px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-blue-700 transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed">Post</button>
            </form>
        </div>
    );
};
...
const TaskCard = ({ task, onEdit, onDelete, onDragStart, onDragEnd }) => {
    ...
    return (
        <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ type: "spring", stiffness: 300, damping: 25 }}
            className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:ring-2 hover:ring-blue-500 transition-all duration-200 flex flex-col cursor-grab active:cursor-grabbing"
            draggable="true" onDragStart={(e) => onDragStart(e, task.id, task.status)} onDragEnd={onDragEnd}>
            <div className="flex items-start gap-3">
                <span className={`mt-1.5 h-2.5 w-2.5 flex-shrink-0 rounded-full ${priorityStyle.dot}`}></span>
                <div className="flex-grow">
                    <h3 className="text-base font-semibold text-gray-800 break-words">{task.title}</h3>
                    <p className="text-sm text-gray-500 break-words line-clamp-2 mt-1">{task.description || 'No description provided.'}</p>
                </div>
            </div>
            <div className="flex justify-between items-center text-xs mt-4 pt-3 border-t border-gray-100">
                <p className="text-gray-500">Assignee: <span className="font-medium text-gray-700">{task.assignee || 'Unassigned'}</span></p>
                <div className="flex items-center space-x-1">
                    <span className={`px-2 py-0.5 font-semibold rounded-md text-xs ${priorityStyle.badge}`}>{task.priority}</span>
                    <button onClick={() => onEdit(task)} className="text-gray-400 hover:text-blue-600 p-1.5 rounded-full hover:bg-gray-100 transition" title="Edit Task"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L14.732 3.732z" /></svg></button>
...
// Inside the TaskModal component...
const handleQuickStatusUpdate = (newStatus) => {
  setStatus(newStatus);
  if (initialTask) {
      // This line is already here
      onSubmit({ status: newStatus }, initialTask.id);
      
      // ✨ FIX: Add this line to close the modal immediately after updating.
      onClose();
  }
}; 
...
const CreateGroupModal = ({ isOpen, onClose, onCreate }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        onCreate(groupName, inviteEmails);
        setGroupName('');
        setInviteEmails('');
    };
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <motion.div initial={{ y: 50, scale: 0.95 }} animate={{ y: 0, scale: 1 }} exit={{ y: 50, scale: 0.95 }} className="bg-white rounded-2xl w-full max-w-md shadow-xl">
                        <form onSubmit={handleSubmit} className="p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label htmlFor="group-name" className="block text-sm font-medium text-gray-700 mb-1">Workspace Name</label>
                                    <input id="group-name" type="text" value={groupName} onChange={(e) => setGroupName(e.target.value)} required className="w-full p-2 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="e.g., Marketing Team"/>
                                </div>
                                <div>
                                    <label htmlFor="invite-emails-create" className="block text-sm font-medium text-gray-700 mb-1">Invite Members (Optional)</label>
                                    <input id="invite-emails-create" type="text" value={inviteEmails} onChange={(e) => setInviteEmails(e.target.value)} placeholder="email1@example.com, email2@example.com" className="w-full p-2 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                            ...
                            <div className="flex justify-end space-x-3 mt-6 pt-4 border-t border-gray-200">
                                ...
                                <button type="submit" disabled={!groupName.trim()} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg shadow-sm hover:bg-blue-700 transition disabled:opacity-50">Create</button>
                            </div>
                        </form>
                    </motion.div>
                </motion.div>
...
const ManageWorkspaceModal = ({ isOpen, onClose, currentGroupId, currentUserId }) => {
    ...
    useEffect(() => {
        if (!isOpen || !db || !currentGroupId) return;
        setIsLoading(true);
        const membersRef = collection(db, `artifacts/${appId}/public/data/groups/${currentGroupId}/members`);
    ...
    const handleInvite = async (e) => {
        ...
        console.log(`Inviting emails: ${emails.join(', ')} to group ${currentGroupId}`);
        alert(`Invitation logic triggered for:\n${emails.join('\n')}\n\nIn a real app, this would send an email with the link:\n${shareableLink}`);
        setInviteEmails('');
    };
...
const TaskManagementView = ({ currentUserId, onLogout, currentUserEmail }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const unsub = onSnapshot(query(collection(db, collectionPath), orderBy('createdAt', 'desc')), (snap) => {
            const newTasks = snap.docs.map(d => ({ id: d.id, ...d.data() }));
            setTasks(newTasks); setLoading(false);
...
// ✨ FIX: This function now correctly uses the helper for all cases.
...
// --- ROOT APP COMPONENT (Handles Auth State) ---
const App = () => {
